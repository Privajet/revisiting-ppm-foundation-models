#!/bin/bash
#SBATCH --job-name=llm-peft-ppm_tabpfn3_error_analysis
...
# Load secrets
if [[ -f "$HOME/.config/llm-peft-ppm/tabpfn.env" ]]; then
  source "$HOME/.config/llm-peft-ppm/tabpfn.env"
else
  echo "Missing secret file: $HOME/.config/llm-peft-ppm/tabpfn.env"
  exit 1
fi
...
# Configuration
PARAMS_FILE="scripts/tabpfn3_params.txt"
PY_MAIN="fertig_lennart_next_event_prediction.py"
PROJECT="llm-peft-ppm_tabpfn3_baseline"
